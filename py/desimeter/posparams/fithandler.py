...
output_columns = ['POS_ID', 'ANALYSIS_DATE_STATIC', 'ANALYSIS_DATE_DYNAMIC',
                  'NUM_POINTS', 'FIT_ERROR_STATIC', 'FIT_ERROR_DYNAMIC',
                  'DATA_START_DATE', 'DATA_END_DATE',
                  'DATA_START_DATE_SEC', 'DATA_END_DATE_SEC']
output_columns += fitter.static_keys
output_columns += [key + '_STATIC' for key in fitter.dynamic_keys]
output_columns += [key + '_DYNAMIC' for key in fitter.dynamic_keys]

def run_best_fits(table, datum_dates, data_window, savedir, static_quantile, verbose=True):
    ...
    INPUTS:
        ...
        verbose ... boolean, affects how much is printed to stdout
    '''
    ...
    these_best = {key:np.median(selection[key]) for key in fitter.static_keys}
    ...
    merged.write(merged_filepath)
    print(f'{posid}: Best-fit results written to {merged_filepath}')
...
def _define_cases(table, datum_dates, data_window):
    ...
    for J in range(len(final_idxs)):
        ...
        cases.append(case)
    posid = table['POS_ID'][0]
    print(f'{posid}: {len(cases):5d} analysis cases defined', flush=True)
    return cases

def _process_cases(table, cases, mode, param_nominals, verbose=True):
    ...
    OUTPUT:
        output ... astropy table of results, headers given by output_columns
    '''
    posid = table['POS_ID'][0]
    ...
    for case in cases:
        ...
        mode_suffix = '_' + mode.upper()          
        output['ANALYSIS_DATE' + mode_suffix].append(Time.now().iso)
