...
parser.add_argument('date', type=str, help='MMDD date string', nargs='+')
...
for date in opts.date:
    ...
    for expid in expids:
        for cam in cams:
            ...
            matchdists.append((cam, expid, 3600. * np.hypot(dra, ddec)))
    ...
    for e in E:
        dists = []
        for cam,expid,dist in matchdists:
            if expid != e:
                continue
            dists.extend(dist)
        plt.hist(dists, label=e, **ha)
    plt.legend()
